...
void car_img_slc_init(){
    // TCP 需要在主线程中打开
    mhandle = tcp_commu_init_client(15513, "192.168.1.20", 2048*2048*3, func_call, NULL, 0);
}

void car_slc_send(float *val){
    //  TCP发送
    unsigned char *array = new unsigned char[4*FRAME_LENGTH];
    for(unsigned char i =0; i< FRAME_LENGTH; i++){
        float2char(val[i], (array+i*4));
    }
    tcp_commu_send(mhandle, array, 4*FRAME_LENGTH);
    delete array;
}
